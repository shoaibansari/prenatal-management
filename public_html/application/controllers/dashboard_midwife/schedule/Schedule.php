...
class Schedule extends CI_Controller {
	...
	public function __construct()
	{
		...
		$this->load->model(array(
			'dashboard_midwife/schedule/schedule_model',
	...
	public function index()
	{
		...
		$data['schedules'] = $this->schedule_model->read_by_nurse(
			$this->session->userdata('user_id')
		);
		$data['content'] = $this->load->view('dashboard_midwife/schedule/view',$data,true);
	...
	{  
		...
		if ($this->form_validation->run() === true) {
			...
			if (empty($postData['schedule_id'])) {
				...
				redirect('dashboard_midwife/schedule/schedule/create');
			} else {
				...
				redirect('dashboard_midwife/schedule/schedule/edit/'.$postData['schedule_id']);
			}

		} else {
			$data['doctor_list'] = $this->doctor_model->doctor_list();
			$data['content'] = $this->load->view('dashboard_midwife/schedule/form',$data,true);
			$data['count']    = $this->dashboard_model->getAlertCount(); 
	...
	{ 
		...
		$data['schedule'] = $this->schedule_model->read_by_doctor_id($schedule_id, $nurse_id);
		$data['doctor_list'] = $this->doctor_model->doctor_list();
		$data['content'] = $this->load->view('dashboard_midwife/schedule/form',$data,true);
		$data['count']    = $this->dashboard_model->getAlertCount(); 
	...
	{ 
		...
		redirect('dashboard_midwife/schedule/schedule/');
	}
