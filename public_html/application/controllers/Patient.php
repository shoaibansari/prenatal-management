...
class Patient extends CI_Controller {
	...
	public function create()
	{
		...
		#-------------------------------#
		$this->form_validation->set_rules('firstname', display('first_name'),'required|max_length[50]|alpha');
		$this->form_validation->set_rules('middlename', 'Middle Name','required|max_length[50]|alpha');
		$this->form_validation->set_rules('lastname', display('last_name'),'required|max_length[50]|alpha');
		...
		$this->form_validation->set_rules('password', display('password'),'required|min_length[8]|max_length[32]');
		$this->form_validation->set_rules('date_of_birth', display('date_of_birth'),'required|max_length[10]');
		$this->form_validation->set_rules('name_of_village', 'Name of Village','required|max_length[20]|regex_match[/^[A-Z a-z \-]+$/]');
		$this->form_validation->set_rules('quarter', 'Quarter','required|max_length[10]');
		$this->form_validation->set_rules('phone', display('phone'),'required|max_length[13]|number');
		$this->form_validation->set_rules('income', 'Income Per Month','required|number');
		...
		$this->form_validation->set_rules('occupation', 'Occupation','required|max_length[50]|alpha');
		...
		if ($this->form_validation->run() === true) {
	      ...
	      //           }
				...
				redirect('patient/profile/' . $insert_id);
			} else {
    ...
    */
    ...
    public function do_upload()
    {
        ...
        if (($_SERVER['REQUEST_METHOD']) == "POST") { 
            ...
            $config['allowed_types'] = 'pdf|doc|docx|bmp|gif|jpg|jpeg|png';
