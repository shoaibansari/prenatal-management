...
class Schedule_model extends CI_Model {
	...
	public function read_by_nurse($nurse_id = null)
	{		
		return $this->db->select("schedule.*, user.firstname, user.lastname, department.name")
			->from($this->table)
			->join('user','user.user_id = schedule.doctor_id','left')
			->join('department','department.dprt_id = user.department_id','left')
			->where('schedule.created_by',$nurse_id)
			->get() 
			->result();
	} 
	...
	public function read_by_doctor_id($schedule_id = null, $nurse_id = null)
	{
		return $this->db->select("*")
			->from($this->table)
			->where('schedule_id',$schedule_id)
			->where('created_by',$nurse_id)
			->get()
			->row();
	} 
